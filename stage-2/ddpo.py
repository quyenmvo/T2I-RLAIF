...
class LLaVAScorer:
    ...
    def __call__(self, images):
        ...
        while not score and iter < 5:
            respone, _ = getResponeFromLLaVA7b(self.prompt, images)
...
def prompt_fn():
    ...
    def _fn():
        nonlocal i
        i = (i+1)%len(selected_samples)
        return selected_samples[i], {}
    return _fn
...
Answer: """
    ...
    TOTAL_ITERATION = 100
    NUM_SELECTED_SAMPLE = 10
    iteration_list = []
    inception_score_mean_list = []
    frechet_inception_distance_list = []
    with open("valid_prompts.json", 'r') as f:
        valid_prompts = json.load(f)
    prompt_set = valid_prompts
    path_list = [os.path.join("./ground_truth", f"{img_id}.png") for img_id in prompt_set.keys()]
    ...
    for iter in range(1, TOTAL_ITERATION + 1):
        ...
        selected_samples = random.sample(train_dataset, NUM_SELECTED_SAMPLE)
        if iter % 10 == 0:
            trainer.valid(prompt_set, iter)
            path_list = [os.path.join(f"./valid_image_{iter}", f"{img_id}.png") for img_id in prompt_set.keys()]
            ...
            inception_score_mean_list.append(metrics_dict['inception_score_mean'])
            frechet_inception_distance_list.append(metrics_dict['frechet_inception_distance'])
            metrics = {
                "Inception Score Mean": inception_score_mean_list,
                "Frechet Inception Distance": frechet_inception_distance_list,
            }
            for metric_name, metric_values in metrics.items():
                plt.plot(iteration_list, metric_values)
                plt.xlabel('Iteration')
                plt.ylabel(metric_name)
                plt.title(f'{metric_name} over Iterations')
                plt.savefig(f'valid_image_{iter}/{metric_name}.png')
                plt.close()
